...
`cât-de-român-ești` este o aplicație web gratuită, educațională, în limba română,
cu jocuri de cuvinte și cunoștințe. Jocurile pot fi folosite fără cont. Progresul activ,
istoricul afișat, recordurile locale, măiestria și circuitul zilnic rămân în browser. Dacă
alegi „Sign in with Google” și ai consimțământ curent, aplicația poate încărca o copie
privată a rezultatelor terminate. Copia nu este descărcată, restaurată sau combinată
automat pe alt dispozitiv.
...
- cere corectarea sau ștergerea contului și a datelor de pe server;
...
| Hetzner | persoană împuternicită pentru hosting | baza de date cont/copii private/recorduri, loguri server | Germania/Finlanda, UE/SEE `[[CONFIRM: exact region and DPA]]` |
...
Datele de cont și copia privată sunt proiectate să fie găzduite în UE/SEE pe infrastructura
Hetzner. Operatorul nu intenționează să transfere această bază de date în afara SEE. Orice
transfer tehnic posibil prin Google Identity sau Cloudflare trebuie verificat înainte de
lansare și acoperit prin clauze contractuale standard, decizii de adecvare sau alt mecanism
GDPR valid, după caz.
...
Browserul folosește localStorage pentru progresul afișat și preferințe. Copia de cont nu
reface automat acest document local. Aplicația nu folosește cookie-uri de publicitate sau
tracking. Orice cookie sau SDK opțional de analiză, marketing sau tracking trebuie introdus
numai după consimțământ opt-in și după actualizarea acestei politici.
...
`cât-de-român-ești` is a free Romanian-language educational word-game web app. The app can
be used without an account. Active progress, displayed history, local bests, mastery and the
daily circuit stay in the browser. If you choose “Sign in with Google” and have current
consent, the app can upload a private copy of completed-score rows. That copy is not
automatically downloaded, restored or merged on another device.
...
| Hetzner | hosting processor | account/private-score-copy/record database, server logs | Germany/Finland, EU/EEA `[[CONFIRM: exact region and DPA]]` |
...
The account and private-score-copy database is intended to be hosted in the EU/EEA on Hetzner.
The controller does not intend to transfer that database outside the EEA. Any technical
transfers by Google Identity or Cloudflare must be confirmed before launch and covered by
valid GDPR transfer mechanisms.
...
The browser uses localStorage for displayed progress and preferences. The account copy does
not automatically rebuild that local document. The app does not use advertising or tracking
cookies. Any optional analytics, marketing or tracking technology must be opt-in and
reflected in this notice before use.
