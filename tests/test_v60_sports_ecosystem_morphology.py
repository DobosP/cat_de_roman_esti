"""Regression contract for the reviewed V60 sports-ecosystem morphology wave."""
...
import contexto_common_words_v60_data as DATA  # noqa: E402
...
_REJECTED_TARGETS = {
    "fileului": "n_v4spo_fileu",
    "fileurilor": "n_v4spo_fileu",
}
_REJECTED = set(_REJECTED_TARGETS)
_HISTORICAL_NONACCEPTED = {
    # V51 deferred, rejected, and unauthored spellings.
    "amicii",
    "arborele",
    "arbori",
    "arborii",
    ...
    "wifi",
    "email",
    # V52 through V59 rejected polysemes.
    "păturile",
    "păturilor",
    "mesei",
    "meselor",
...
def test_v60_review_funnel_is_exact_complete_and_collision_aware() -> None:
    ...
    assert review["schema"] == "v60-sports-ecosystem-morphology-review-v1"
    ...
    collision_evidence = [
        "sports net",
        "textile, fishing, or shopping net",
        "hair net",
        "meat or fish fillet (homographic file)",
    ]
    assert review["sense_collision_evidence"] == {
        "fileului": collision_evidence,
        "fileurilor": collision_evidence,
    }
...
def test_v60_alias_batch_is_exact_collision_free_and_applied_to_both_mirrors() -> None:
    fixture = _json(_PACKAGE_KG)
    svc = WordGameService(load_fixture(_PACKAGE_KG).graph)
    aliases = _accepted_aliases()

    assert DATA.BUILD_VERSION == "fixture-v60-sports-ecosystem-morphology"
    ...
    assert svc.resolve("golurilor marcate") == "n_v3spo_gol"
    assert svc.resolve("cluburilor sportive") == "n_v2spo_club_sportiv"
    assert svc.resolve("porții de joc") == "n_v4spo_poarta"
...
def test_v60_nonaccepted_surfaces_stay_out_of_typed_games() -> None:
    client = Client()
    svc = get_service()
    ...
    assert len(_HISTORICAL_NONACCEPTED) == 31
...
def test_v60_preserves_projection_topology_pack_and_frozen_board_payloads() -> None:
    fixture = _json(_PACKAGE_KG)
...
def test_v60_mobile_contract_and_v49_ledger_persist_exactly() -> None:
