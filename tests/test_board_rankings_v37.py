...
def test_every_eligibility_flag_matches_payload_and_critique_gates(generated: dict) -> None:
    pack, svc, strong, regions = RANK.critique_pack.load_all(
        RANK.critique_pack.PACKAGE_PACK,
        RANK.critique_pack.PACKAGE_KG,
    )
    _, _, selected = RANK.critique_pack.run(
        pack,
        svc,
        strong,
        regions,
        list(RANK.GAME_KINDS),
        set(RANK.STATUSES),
        None,
    )
    expected = {
        str(rec["id"]): (
            ...
            and not RANK.validate_payload(rec, game, svc)
            and not any(finding.get("level") == "FAIL" for finding in findings)
        )
        for game, rec, findings in selected
    }

    demotions = set(RANK._owner_demotions())
    expected = {
...
def test_default_checker_is_green_and_prints_a_human_audit(capsys) -> None:
    ...
    assert "724 total / 447 pilot-eligible" in output
