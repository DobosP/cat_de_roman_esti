...
def _ranking_document() -> dict:
    ...
    for game in GAME_KINDS:
        game_boards = []
        for rank, rec in enumerate(sorted(pack[game], key=lambda item: item["id"]), 1):
        ...
        eligible = [entry for entry in game_boards if entry["pilot_eligible"]]
        for index, entry in enumerate(eligible):
            entry["selection_weight"] = 5 - min(4, (5 * index) // len(eligible))
...
def test_pinned_rubric_digest_matches_repository_source():
    assert DEFAULT_RUBRIC_SHA256 == normalized_text_sha256(DEFAULT_RUBRIC)


def test_installed_package_without_docs_still_validates_rubric_digest(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    valid = _write_rankings(tmp_path / "valid.json")
    monkeypatch.setattr(
        "cat_de_roman_esti.wordgames.packs.DEFAULT_RUBRIC",
        tmp_path / "docs-not-shipped.md",
    )
    assert load_pack(DEFAULT_PACK, rankings_path=valid).pool("contexto")

    document = _ranking_document()
    document["meta"]["rubric_sha256"] = "0" * 64
    drifted = _write_rankings(tmp_path / "drifted.json", document)
    with pytest.raises(ValueError, match="does not match the reviewed rubric"):
        load_pack(DEFAULT_PACK, rankings_path=drifted)


def test_bundled_pack_requires_its_ranking_sidecar(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    monkeypatch.delenv("CAT_BOARD_RANKINGS", raising=False)
    monkeypatch.delenv("CAT_KG_FIXTURE", raising=False)
    monkeypatch.setattr(
        "cat_de_roman_esti.wordgames.packs.DEFAULT_RANKINGS",
        tmp_path / "missing-rankings.json",
    )

    with pytest.raises(ValueError, match="board rankings: cannot read"):
...
def test_rankings_score_formula_drift_fails_closed(tmp_path: Path):
    document = _ranking_document()
    document["boards"][0]["pilot_score"] -= 1
    rankings = _write_rankings(tmp_path / "rankings.json", document)

    with pytest.raises(ValueError, match="pilot_score does not match"):
        load_pack(DEFAULT_PACK, rankings_path=rankings)


def test_rankings_order_drift_fails_closed(tmp_path: Path):
    document = _ranking_document()
    same_game = [
        entry for entry in document["boards"] if entry["game"] == GAME_KINDS[0]
    ]
    same_game[0]["rank"], same_game[1]["rank"] = (
        same_game[1]["rank"],
        same_game[0]["rank"],
    )
    rankings = _write_rankings(tmp_path / "rankings.json", document)

    with pytest.raises(ValueError, match="ranks must follow"):
        load_pack(DEFAULT_PACK, rankings_path=rankings)


def test_rankings_quintile_drift_fails_closed(tmp_path: Path):
    document = _ranking_document()
    first_eligible = next(
        entry for entry in document["boards"] if entry["pilot_eligible"] is True
    )
    first_eligible["selection_weight"] = 4
    rankings = _write_rankings(tmp_path / "rankings.json", document)

    with pytest.raises(ValueError, match="selection_weight does not match"):
        load_pack(DEFAULT_PACK, rankings_path=rankings)
...
def test_rankings_schema_version_requires_exact_integer(tmp_path: Path, invalid: object):
    document = _ranking_document()
    document["meta"]["schema_version"] = invalid
    rankings = _write_rankings(tmp_path / "rankings.json", document)
...
def test_rankings_counts_require_exact_integer_types(tmp_path: Path):
    document = _ranking_document()
    document["meta"]["counts"]["total"] = 794.0
    rankings = _write_rankings(tmp_path / "rankings.json", document)

    with pytest.raises(ValueError, match="counts must use non-negative integers"):
        load_pack(DEFAULT_PACK, rankings_path=rankings)
