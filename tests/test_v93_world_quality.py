"""New kitchen words preserve every earned book and add useful preparation routes."""
...
NEW_WORDS = {
    ...
    "alw_food_clatite_gratinate": "Clătite gratinate",
    "alw_food_galuste_prune": "Găluște cu prune",
    "alw_food_milkshake": "Milkshake",
    "alw_food_musaca": "Musaca",
    "alw_food_oua_umplute": "Ouă umplute",
    "alw_food_pesmet": "Pesmet",
    "alw_food_salata_paste": "Salată de paste",
    "alw_food_supa_taitei": "Supă cu tăiței",
}
HISTORY = [
    ...
    (225, 121, PREVIOUS_SHA),
]
NEW_RECIPES = (
    "oua-umplute-maioneza", "oua-umplute-pate", "musaca-cartof-carne", "musaca-piure-carne",
    "clatite-gratinate-smantana", "clatite-gratinate-cuptor", "galuste-prune-cartof",
    "galuste-prune-oparit", "milkshake-inghetata-lapte", "milkshake-banana-lapte",
    "bruschete-paine-prajita-rosie", "bruschete-paine-rosie", "salata-paste-legume",
    "salata-paste-legume-gratar", "budinca-paine-lapte", "budinca-paine-stafide",
    "pesmet-paine-prajita-sucitor", "pesmet-crutoane-sucitor", "snitel-pesmet-carne",
    "snitel-pesmet-pui", "supa-taitei-paste",
)
...
def test_additions_preserve_prior_records_and_create_reusable_bread_preparation():
    ...
    assert len(current["compatible_versions"]) == 5
    counts = Counter(r["result"] for r in current["recipes"] if r["result"] in NEW_WORDS)
    assert counts == {cid: 1 if cid == "alw_food_supa_taitei" else 2 for cid in NEW_WORDS}
    ...
    assert (results_before - used_before) & used_now == {
        "alw_food_crutoane", "alw_food_legume_gratar", "n_v3gas_clatite", "n_v3gas_inghetata",
    }
    assert "alw_food_pesmet" in used_now
...
def test_every_new_recipe_and_alternative_works_in_either_order_without_duplicate_credit(
    ...
    state = post(data={"progress": progress})
    if recipe_id.startswith("snitel-pesmet"):
        state = craft(state, world, "pesmet-crutoane-sucitor")
...
def test_completed225_book_gains_ten_words_without_losing_its_earned121_entries(world):
    ...
    for recipe_id in (
        "oua-umplute-pate", "musaca-piure-carne", "clatite-gratinate-cuptor",
        "galuste-prune-oparit", "milkshake-inghetata-lapte", "bruschete-paine-prajita-rosie",
        "salata-paste-legume-gratar", "budinca-paine-lapte", "pesmet-crutoane-sucitor",
        "supa-taitei-paste",
    ):
        state = craft(state, world, recipe_id)
        assert len(state["discovered"]) == 1
    assert state["complete"] and state["discovered_count"] == 131
    assert len(state["inventory"]) == 235
    assert {item["id"] for item in state["inventory"]} - old_owned == set(NEW_WORDS)
    assert state["progress"]["discoveries"][:121] == progress["discoveries"]
...
def test_225_book_cannot_claim_new_recipe_before_upgrade(world):
    progress, owned = complete_old_checkpoint(world.catalog.compatible_versions[-1])
    pair, _ = next((pair, recipe) for pair, recipe in world.recipes.items()
                   if recipe.id == "oua-umplute-pate")
