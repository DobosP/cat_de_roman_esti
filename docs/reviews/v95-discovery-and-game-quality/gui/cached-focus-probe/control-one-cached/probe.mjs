...
const OUT='/home/dobo/work/_temp/feat__v95-discovery-and-game-quality/gui/cached-focus-probe/control-one-cached';
...
try{
 for(const mode of ['committed-response-loss','uncommitted-response-loss']){
  ...
  for(let n=1;n<=1;n++){await word(page,'Cheag alimentar').focus();await page.keyboard.press('Enter');await expect(page.locator('.alchemy-feedback')).toContainText('ai încercat deja');await expect(word(page,'Cheag alimentar')).toBeFocused();checkpoints.push({step:'cached-'+n,focus:await focus(page)});}
