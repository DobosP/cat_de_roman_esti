...
export async function start(page, game) {
...
export async function act(page, game, step) {
  const response = page.waitForResponse((r) =>
  ...
  if (game.key === "contexto" || game.key === "lant") {
    ...
    if (game.key === "alchimie") {
      await page.getByRole("button", { name: "Combină cele două concepte selectate" }).click();
...
export async function solve(page, game, steps) {
  let result;
