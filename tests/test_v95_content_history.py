"""V95 additions preserve exact reviewed V94 content and reject forged inverses."""
...
from tests.content_history import before_v95_artifact, before_v96_artifact
...
def test_v95_inverse_restores_complete_cf6b28b_bytes_without_mutating_input(filename):
    ...
    if filename == "games_pack.json":
        assert sum(len(previous[g]) for g in ADDED) == 705
        ...
        assert len(old) == 705 and set(new) - set(old) == set().union(*ADDED.values())
...
def test_v95_inverse_rejects_unreviewed_artifact_bytes(filename, part):
    ...
    with pytest.raises(AssertionError):
        before_v95_artifact(current, filename)


@pytest.mark.parametrize("change", ["remove_added", "modify_added", "extra_row", "rank_weight"])
def test_v95_inverse_checks_additions_and_rank_changes_before_stripping(change):
    ...
    if change == "remove_added":
        current["conexiuni"] = [r for r in current["conexiuni"]
                                if r["id"] != "cx_viata_de_roman_372"]
    elif change == "modify_added":
        row = next(r for r in current["lant"] if r["id"] == "lt_gastronomie_243")
...
def test_v95_receipt_cannot_be_rebound_to_tampered_content(tmp_path, monkeypatch):
    filename = "games_pack.json"
    current = read_current(filename)
    current["meta"]["unreviewed"] = True
    receipt = json.loads(content_history._V95_RECEIPT.read_bytes())
    receipt["files"][filename]["after_sha256"] = digest(current, filename)
    receipt["files"][filename]["head_after"]["meta"]["unreviewed"] = True
    path = tmp_path / "forged-v95-receipt.json"
    path.write_text(json.dumps(receipt, ensure_ascii=False, indent=2) + "\n")
    monkeypatch.setattr(content_history, "_V95_RECEIPT", path)
    with pytest.raises(AssertionError):
        before_v95_artifact(current, filename)


def test_v95_keeps_v94_rejection_ledger_exact_and_caraiman_absent():
    ...
    assert pack["meta"]["id_high_water"]["lant"] == 244
