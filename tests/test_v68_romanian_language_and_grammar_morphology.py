"""Regression contract for the reviewed V68 Romanian-language-and-grammar wave."""
...
import contexto_common_words_v68_data as DATA  # noqa: E402

_HISTORICAL_ALIAS_MODULES = tuple(
    importlib.import_module(f"contexto_common_words_v{version}_data") for version in range(51, 68)
)
...
# Current V70 whole-artifact pins; the immutable payload pins below must not move.
_CURRENT_BUILD_VERSION = "fixture-v70-social-and-civic-life-morphology"
...
_CANDIDATE_FUNNEL_SHA256 = "ff4cee7a4ac2f39601a19efd3aad2a305e85303e282451ba606fe1c8b68b3377"
...
_REJECTED_TARGETS = {
    "punctului": "n_v4lim_punct",
    "punctelor": "n_v4lim_punct",
}
_REJECTED = set(_REJECTED_TARGETS)
_EXPECTED_LEXICAL_SOURCES = (
    "https://dexonline.ro/definitie/sunet",
    "https://dexonline.ro/definitie/scriere",
    "https://dexonline.ro/definitie/limb%C4%83",
    "https://dexonline.ro/definitie/vocabular",
    "https://dexonline.ro/definitie/parte%2Bde%2Bvorbire",
    "https://dexonline.ro/definitie/proverb",
    "https://dexonline.ro/definitie/neologism",
    "https://dexonline.ro/definitie/omonim",
    "https://dexonline.ro/definitie/vorbire",
    "https://dexonline.ro/definitie/argou",
    "https://dexonline.ro/definitie/sintax%C4%83",
    "https://dexonline.ro/definitie/paronim",
    "https://dexonline.ro/definitie/gramatic%C4%83",
    "https://dexonline.ro/definitie/virgul%C4%83",
    "https://dexonline.ro/definitie/flexiune",
    "https://dexonline.ro/definitie/diminutiv",
    "https://dexonline.ro/definitie/expresie",
    "https://dexonline.ro/definitie/persoan%C4%83",
    "https://dexonline.ro/definitie/plural",
    "https://dexonline.ro/definitie/gre%C8%99eal%C4%83",
    "https://dexonline.ro/definitie/grai",
    "https://dexonline.ro/definitie/fraz%C4%83",
    "https://dexonline.ro/definitie/dialect",
    "https://dexonline.ro/definitie/ortografie",
    "https://dexonline.ro/definitie/punct",
)
_V51_DEFERRED = {
    "amicii",
    "arborele",
...
_HISTORICAL_WAVE_BLOCKS = {
    # V52 through V67 rejected polysemes.
    "păturile",
    "păturilor",
    "mesei",
    "meselor",
...
def _historical_accepted_aliases() -> dict[str, str]:
    ...
    assert len(bindings) == 798
    assert len({alias for alias, _node_id in bindings}) == 798
...
def test_v68_review_funnel_is_exact_complete_and_collision_aware() -> None:
    ...
    assert review["schema"] == ("v68-romanian-language-and-grammar-morphology-review-v1")
    ...
    collision_evidence = [
        "punctuation mark or sentence-final full stop",
        "geometric point, place, position, or temporal point",
        "score, measurement unit, separator, list item, viewpoint, or focus",
    ]
    assert review["sense_collision_evidence"] == {
        "punctului": collision_evidence,
        "punctelor": collision_evidence,
    }
    ...
    assert review["lexical_sources"] == list(_EXPECTED_LEXICAL_SOURCES)
...
def test_v68_alias_batch_is_exact_collision_free_and_applied_to_both_mirrors() -> None:
    ...
    assert DATA.BUILD_VERSION == ("fixture-v68-romanian-language-and-grammar-morphology")
    ...
    assert all(svc.node(node_id).category == "limba" for node_id in owners)
    assert all(
        4
        <= sum(
    ...
    assert svc.resolve("sunetelor limbii române") == "n_v4lim_sunet"
    assert svc.resolve("gramaticii limbii române") == "n_gramatica"
    assert svc.resolve("virgulelor ortografice") == "n_v4lim_virgula"
    assert svc.resolve("pluralelor gramaticale") == "n_v4lim_plural"
    assert svc.resolve("ortografiilor limbilor romanice") == "n_ortografie"
...
def test_v68_nonaccepted_surfaces_stay_out_of_typed_games() -> None:
    client = Client()
    svc = get_service()
    ...
    assert len(_HISTORICAL_WAVE_BLOCKS) == 34
    assert len(_HISTORICAL_NONACCEPTED) == 47
    assert len(_HISTORICAL_NONACCEPTED | _REJECTED) == 49
...
def test_v68_preserves_projection_topology_pack_and_frozen_board_payloads() -> None:
    fixture = _json(_PACKAGE_KG)
...
def test_v68_mobile_contract_and_v49_ledger_persist_exactly() -> None:
