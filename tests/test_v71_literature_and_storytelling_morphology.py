"""Regression contract for the reviewed V71 literature-and-storytelling wave."""
...
import contexto_common_words_v71_data as DATA  # noqa: E402

# V69 changes selection/ranking but has no vocabulary data module. V70 resumes
# the alias ledger after the historical V51-V68 modules.
...
_REVIEW = _ROOT / "docs/reviews/v71-literature-and-storytelling-morphology/vocabulary.json"

# Exact V70 baselines. The V71 transaction must move the alias-bearing artifacts only.
...
# Exact V71 historical pins remain fixed after the V72 transaction.
...
# Current shared wrapper pins; V71's review and source assertions remain historical.
...
_CANDIDATE_FUNNEL_SHA256 = "85e26519b648dbf28813a150955305c223feb5d627199d5ba689d709c526c64a"
_ACCEPTED_MAP_SHA256 = "91c1c76b3ee8000114ba1b615b67b7f9b1bc5cf4a8f8d0135d17bdb60fbb2e8b"
...
_REJECTED_TARGETS = {
    "intrigii": "n_v4lit_intriga",
    "intrigilor": "n_v4lit_intriga",
}
...
def _historical_accepted_aliases() -> dict[str, str]:
    ...
    assert len(bindings) == 892
    assert len({alias for alias, _node_id in bindings}) == 892
...
def test_v71_review_funnel_is_exact_complete_and_unanimous() -> None:
    ...
    assert review["schema"] == "v71-literature-and-storytelling-morphology-review-v1"
...
def test_v71_alias_batch_is_exact_collision_free_and_applied_to_both_mirrors() -> None:
    ...
    assert DATA.BUILD_VERSION == "fixture-v71-literature-and-storytelling-morphology"
    ...
    assert all(svc.node(node_id).category == "literatura" for node_id in owners)
    assert all(
        4
        <= sum(
    ...
    assert svc.resolve("avangardelor literare") == "n_v2lit_avangarda"
    assert svc.resolve("dramaturgiilor") == "n_v2lit_dramaturgie"
    assert svc.resolve("mesajelor din operele literare") == "n_v4lit_mesaj"
    assert svc.resolve("satelor din literatura română") == "n_v2lit_sat_romanesc"
    assert svc.resolve("zmeilor din basme") == "n_v23lit_zmeul"
...
def test_v71_aliases_play_in_contexto_and_only_on_existing_legal_lant_hops() -> None:
    client = Client()
    svc = get_service()
...
def test_v71_fuzzy_deny_is_exact_bounded_and_keeps_advisory_help() -> None:
    svc = get_service()
    assert (
        {normalize(surface) for surface in REVIEWED_FUZZY_DENY_SURFACES}
        == {normalize(surface) for surface in DATA.BLOCKED_ALIAS_FORMS}
        == _REJECTED
    )
    ...
    assert all(svc.suggest(surface) == ["intrigă"] for surface in _REJECTED)
    assert svc.resolve("strugur") is None
    assert svc.resolve_fuzzy("strugur") == "n_v24_food_small_fruit_strugure"


def test_v71_nonaccepted_surfaces_stay_out_of_typed_games() -> None:
    client = Client()
    svc = get_service()
    for surface, target in sorted(_REJECTED_TARGETS.items()):
        ...
        assert rejected_move["suggestions"] == ["intrigă"]
        unchanged = lant_store.get(lant_id)
        assert unchanged.chain == [start]
        assert unchanged.moves == 0
        assert unchanged.won is False

        exact_move = client.post(
            f"/api/wordgames/lant/games/{lant_id}/move",
            {"text": "intrigă"},
            content_type="application/json",
        ).json()
        assert exact_move["ok"] is True
        assert exact_move["won"] is True

    assert len(DATA.BASE_DEFERRED_AMBIGUOUS_TERMS) == 68
    assert len(DATA.DEFERRED_AMBIGUOUS_TERMS) == 70
    assert len(set(DATA.DEFERRED_AMBIGUOUS_TERMS)) == 70


def test_v71_preserves_v70_selection_projection_topology_and_frozen_payloads() -> None:
    fixture = _json(_PACKAGE_KG)
    ...
    assert _KG_SHA256 != _V71_KG_SHA256 != _V70_KG_SHA256
    assert _sha256(_PACKAGE_PACK) == _PACK_SHA256
    assert _sha256(_PACKAGE_RANKINGS) == _RANKINGS_SHA256
    assert _RANKINGS_SHA256 != _V71_RANKINGS_SHA256 != _V70_RANKINGS_SHA256
    assert _sha256(_PACKAGE_DERIVED) == _DERIVED_SHA256
    assert _DERIVED_SHA256 != _V71_DERIVED_SHA256 != _V70_DERIVED_SHA256
...
def test_v71_mobile_contract_and_v49_ledger_persist_exactly() -> None:
    ...
    assert _MOBILE_SHA256 != _V71_MOBILE_SHA256 != _V70_MOBILE_SHA256
